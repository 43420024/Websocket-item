...
public class GiftBackpackServiceImpl extends ServiceImpl<GiftBackpackMapper, GiftBackpack>
    ...
    public Result select(Long userId) {
        QueryWrapper<GiftBackpack> wrapper = new QueryWrapper<>();
        wrapper.eq("user_id", userId);
        List<GiftBackpack> giftBackpacks = this.baseMapper.selectList(wrapper);
        for (GiftBackpack giftBackpack : giftBackpacks) {
            final Gift gift = giftService.getById(giftBackpack.getGiftId());
            giftBackpack.setGift(gift);
        }
        return Result.success(giftBackpacks);
    }
}
